    ...
    public partial class ArrayFlight : UserControl
    {
        ...
        private void LoadData()
        {
            ...
            while (reader.Read())
            {
                ...
                data[data.Count - 1][2] = Convert.ToDateTime(reader[2]).ToString("dd.MM.yyyy HH:mm", culture);
        ...
        private void guna2Button1_Click(object sender, EventArgs e)
        {
            ...
            string query = "UPDATE `evdokCvc`.`flight` SET `id_airplane` = @id_airplane, `time_departure` = '"+ Convert.ToDateTime(guna2DataGridView1.Rows[guna2DataGridView1.CurrentRow.Index].Cells[2].Value).ToString("yyyy.MM.dd HH:mm:00", culture) + "' , `departure` = @departure, `arrival` = @arrival where `id_flight` = " + guna2DataGridView1.Rows[guna2DataGridView1.CurrentRow.Index].Cells[0].Value.ToString();
            MySqlCommand command = new MySqlCommand(query, bd.conn);
            ...
            command.Parameters.Add("@arrival", MySqlDbType.VarChar, 455);
